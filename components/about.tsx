...
export default function About() {
  ...
  const tabContent: TabContentMap = {
    mission: {
      ...
      icon: <Shield className="w-5 h-5 sm:w-5 sm:h-5" />,
    ...
    vision: {
      ...
      icon: <Lightbulb className="w-5 h-5 sm:w-5 sm:h-5" />,
  ...
  return (
    <section className="min-h-screen py-8 sm:py-6 md:py-8 lg:py-12 xl:py-16 relative overflow-hidden bg-white">
      {/* Background Elements */}
      ...
      <div className="container mx-auto px-5 sm:px-4 md:px-6 lg:px-8 relative z-10 max-w-7xl">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 sm:gap-8 md:gap-12 lg:gap-16 items-start px-4 sm:px-0">
          {/* Left Content */}
          <div className="space-y-6 sm:space-y-6 md:space-y-8 order-2 lg:order-1">
            {/* Header */}
            <div className="space-y-4 sm:space-y-4 md:space-y-6">
              <div className="flex items-center space-x-3 sm:space-x-3 group">
                <div className="w-10 sm:w-10 md:w-12 h-0.5 bg-gradient-to-r from-blue-500 to-purple-500 rounded-full group-hover:w-12 sm:group-hover:w-12 md:group-hover:w-16 transition-all duration-300"></div>
                <span className="text-blue-600 font-semibold text-sm sm:text-sm tracking-widest uppercase flex items-center space-x-2 sm:space-x-2">
              ...
              <h2 className="text-2xl sm:text-2xl md:text-3xl lg:text-4xl xl:text-5xl font-bold text-gray-800 leading-tight">
                Emerging{" "}
            ...
            <div className="border-b border-gray-300">
              <div
                className="flex space-x-4 sm:space-x-4 md:space-x-6 overflow-x-auto scrollbar-hide pb-2"
              ...
              >
                {Object.entries(tabContent).map(([key, content]) => (
                  <button
                    ...
                    className={`pb-3 sm:pb-3 px-2 sm:px-2 border-b-2 font-semibold transition-all duration-300 whitespace-nowrap flex items-center space-x-2 sm:space-x-2 text-sm sm:text-sm md:text-base ${
                      activeTab === key
            ...
            {/* Tab Content */}
            <div className="p-5 sm:p-4 md:p-6 rounded-xl sm:rounded-2xl border border-gray-300 transition-all duration-300 bg-white shadow-sm">
              <div className="flex items-start space-x-3 sm:space-x-3">
                <div
                  className={`w-10 h-10 sm:w-8 sm:h-8 md:w-10 md:h-10 rounded-xl sm:rounded-xl bg-gradient-to-r ${tabContent[activeTab].color} p-2 sm:p-2 flex items-center justify-center shadow-lg flex-shrink-0`}
                >
                  <span className="text-white">
                    {tabContent[activeTab].icon}
                  </span>
                </div>
                <p className="text-gray-800 leading-relaxed font-semibold text-sm sm:text-sm md:text-base flex-1">
                  ...
                  <div className="bg-gradient-to-r from-blue-500 via-blue-700 to-blue-900 hover:from-blue-600 hover:via-purple-600 hover:to-pink-600 text-white pl-5 sm:pl-4 md:pl-6 pr-10 sm:pr-10 md:pr-12 py-3 sm:py-3 md:py-4 lg:py-5  hover:shadow-lg hover:shadow-blue-500/25 transition-all duration-300 flex items-center space-x-2 sm:space-x-2 relative ">
                    ...
                    <div className="absolute -right-2 sm:-right-2 md:-right-3 lg:-right-5 top-1/2 -translate-y-1/2 h-8 w-8 sm:h-8 sm:w-8 md:h-10 md:w-10 lg:h-17 lg:w-17 bg-white rounded-full flex items-center justify-center ">
                      <div className="w-5 h-5 sm:w-5 sm:h-5 md:w-6 md:h-6 lg:w-8 lg:h-8 rounded-full flex items-center justify-center">
                        <Play
                          className="w-3 h-3 sm:w-3 sm:h-3 md:w-4 md:h-4 lg:w-7 lg:h-7 text-black ml-0.5"
                          fill="currentColor"
                        />
                      </div>
                    </div>
          ...
          <div className="relative order-2 lg:order-2 flex flex-col items-center mt-20 sm:mt-40">
            ...
            <div className="relative w-full flex justify-center items-center mb-8 sm:mb-8 md:mb-12 lg:mb-0">
              {/* Blue Circle Background - Responsive */}
              <div className="absolute w-56 h-56 xs:w-56 xs:h-56 sm:w-80 sm:h-80 md:w-96 md:h-96 lg:w-[450px] lg:h-[450px] xl:w-[550px] xl:h-[550px] rounded-full border-[16px] xs:border-12 sm:border-[20px] md:border-[30px] lg:border-[40px] xl:border-[50px] border-blue-600 animate-bounce opacity-80"></div>

              {/* Top Left Badge - Responsive positioning */}
              <div className="absolute -top-10 xs:-top-10 sm:-top-16 md:-top-20 lg:-top-24 xl:-top-28 -left-2 xs:-left-2 sm:-left-4 md:-left-6 bg-blue-600 text-white px-3 xs:px-3 sm:px-4 md:px-6 py-3 xs:py-3 sm:py-4 md:py-6 rounded-sm shadow-lg flex flex-col items-center text-center z-20">
                <div className="text-sm xs:text-sm sm:text-lg md:text-xl lg:text-2xl xl:text-3xl font-black uppercase tracking-widest">
                  From
                </div>
                <div className="text-2xl xs:text-xl sm:text-3xl md:text-4xl lg:text-5xl xl:text-6xl font-bold leading-none">
                  2009
                </div>
                <div className="text-[10px] xs:text-[10px] sm:text-xs md:text-sm font-semibold uppercase tracking-wide">
                  Serving Globally
                </div>
              </div>
            ...
            <div className="w-full max-w-md sm:max-w-md md:max-w-lg lg:absolute lg:-bottom-16 xl:-bottom-20 2xl:-bottom-24 lg:left-1/2 lg:-translate-x-1/2 lg:max-w-md xl:max-w-lg 2xl:max-w-xl bg-white px-5 xs:px-4 sm:px-5 md:px-6 py-5 xs:py-4 sm:py-5 md:py-6 rounded-xl sm:rounded-xl lg:rounded-tr-lg lg:rounded-tl-lg shadow-lg sm:shadow-xl lg:shadow-2xl border border-gray-100 z-30">
              <div className="flex items-start space-x-3 sm:space-x-3 md:space-x-4">
                <div className="p-2.5 xs:p-2 sm:p-2.5 bg-gradient-to-br from-blue-50 to-blue-100 rounded-lg sm:rounded-xl flex-shrink-0">
                  <Award className="w-5 h-5 xs:w-4 xs:h-4 sm:w-5 sm:h-5 md:w-6 md:h-6 text-blue-600" />
                </div>
                <div className="flex-1 min-w-0">
                  <h4 className="font-bold text-gray-900 mb-2 sm:mb-2 text-sm xs:text-sm sm:text-base md:text-lg leading-tight">
                    Empowering Businesses Across Sectors
                  </h4>
                  <p className="text-gray-600 text-sm xs:text-xs sm:text-sm md:text-base leading-relaxed">
